...
file_samples = None # Legacy buffer for PCM files
file_index = 0
current_file_stream = None # New streamer instance
...
class FileStreamer:
    """Reads audio file via ffmpeg subprocess and buffers data."""
    def __init__(self, file_path, sample_rate, channels=1):
        self.file_path = file_path
        self.sample_rate = sample_rate
        self.channels = channels
        self.queue = queue.Queue(maxsize=100) # Buffer ~2-5 seconds ideally
        self.process = None
    ...
    def start(self):
        ...
        try:
            ...
            logger.info(f"Started ffmpeg stream for {self.file_path}")
    ...
    def read(self, frames):
        """Get 'frames' amount of samples. Returns (data, is_finished)."""
        needed = frames
        
        # 1. Fill from internal leftover buffer
        if len(self.buffer) < needed:
            # Try to fetch more from queue
            while len(self.buffer) < needed:
                try:
                    # Non-blocking check first? No, we need data.
                    # But we don't want to block audio callback too long.
                    # Use a small timeout or no wait if possible? 
                    # Actually, better to block briefly or return zeros logic handled by callback?
                    # We'll use get_nowait to avoid blocking callback
                    new_data = self.queue.get_nowait()
                    if new_data is None: # EOF
                        break
        ...
        if len(self.buffer) == 0:
            return np.zeros(0, dtype=np.float32), not self.running and self.queue.empty()

        take = min(len(self.buffer), needed)
        output = self.buffer[:take]
        self.buffer = self.buffer[take:]
        
        is_finished = (len(output) < needed) and (not self.running) and self.queue.empty()
...
def audio_callback(outdata, frames, time, status):
    ...
    # 2. Add file playback data (Stream or Memory)
    with file_lock:
        if current_file_stream and not PAUSED:
            # --- START STREAMING LOGIC ---
            chunk, is_finished = current_file_stream.read(frames)
            if len(chunk) > 0:
                combined_data[:len(chunk), 0] += chunk
                combined_data[:len(chunk), 1] += chunk
            
            if is_finished:
                logger.info("Stream playback finished.")
                current_file_stream.stop()
                current_file_stream = None
                event_queue.put({"type": "ended"})
            # --- END STREAMING LOGIC ---
            
        elif file_samples is not None and not PAUSED:
            # --- START LEGACY MEMORY LOGIC (Pcm files) ---
            ...
            if remaining > 0:
                ...
                file_samples = None
                file_index = 0
                logger.info("Memory playback finished.")
                event_queue.put({"type": "ended"})
            # --- END LEGACY MEMORY LOGIC ---
...
async def play_audio_file(file_path):
    """Load an audio file using background streaming."""
    global file_samples, file_index, PAUSED, current_file_stream
    try:
        ...
        with file_lock:
            ...
            if current_file_stream:
                current_file_stream.stop()
                current_file_stream = None
            file_samples = None # Clear legacy buffer

            # Initialize new streamer
            logger.info(f"Initializing stream for: {file_path}")
            current_file_stream = FileStreamer(file_path, SAMPLE_RATE, channels=1)
            current_file_stream.start()
            PAUSED = False
            
    except Exception as e:
        logger.exception(f"Error starting file stream: {e}")
...
async def play_pcm_file(file_path):
    ...
    try:
        ...
        logger.info(f"Loading raw PCM into memory: {file_path}")
            ...
            if isinstance(message, str):
                try:
                    ...
                    if cmd == "play":
                        ...
                        with file_lock:
                            global file_samples, current_file_stream
                            file_samples = None
                            if current_file_stream:
                                current_file_stream.stop()
                                current_file_stream = None
...
async def main():
    ...
    def run_http():
        ...
        socketserver.ThreadingTCPServer.request_queue_size = 50 # Increase backlog for concurrent requests
        try:
            with socketserver.ThreadingTCPServer(("", 8080), Handler) as httpd:
                logger.info(f"Serving HTTP on port 8080 (dir: {public_dir})")
                httpd.serve_forever()
        except Exception as e:
